    ...
    public List<JNowFood> queryAllFoodForJNow() {
        ...
        try {
            ...
            while(cursor.moveToNext()){
                ...
                jnowFood.setPrice(cursor.getInt(cursor.getColumnIndex("PRICE")));//菜品价格
                jnowFood.setId(cursor.getString(cursor.getColumnIndex("ITCODE")));//菜品编码
                jnowFood.setName(cursor.getString(cursor.getColumnIndex("DES")));//菜品名称
                jnowFood.setUnit(cursor.getString(cursor.getColumnIndex("UNIT")));//菜品单位
                jnowFood.setSortClass(cursor.getInt(cursor.getColumnIndex("CLASS")));//菜品类别
                jnowFood.setState(cursor.getInt(cursor.getColumnIndex("STATE")));//菜品状态
                jnowLists.add(jnowFood);
            }
        } catch (Exception e) {
            Log.d("对接美味不用等", e.toString());
        }finally{
            if(cursor!= null){
                cursor.close();
            }
